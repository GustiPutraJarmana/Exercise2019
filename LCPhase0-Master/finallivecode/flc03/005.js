...
function reportMarket(input) {
  ...
  for (var i = 0; i < input.length; i++) {
    if (!result[input[i].fruit]) {
      result[input[i].fruit] = {
        customer: [],
        sum: 0,
        max: 0,
    ...
    result[input[i].fruit].customer.push(input[i].name);
    result[input[i].fruit].sum += input[i].qty;
    if (result[input[i].fruit].max < input[i].qty) {
      result[input[i].fruit].max = input[i].qty;
    }
    if (result[input[i].fruit].min > input[i].qty) {
      result[input[i].fruit].min = input[i].qty;
    }
  }
  return result;
}
