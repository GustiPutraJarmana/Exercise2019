 ...
 * 
 * Instruction:
 * 1. Buat fungsi train untuk merubah nama pokemon dan type-nya (jika ada perubahan type) yang di-train ketika level pokemon tersebut sudah mencapai level sesuai
...
function train(pokemon, candy) {
  // code here
  let nextLevel = pokemon['level'] + candy
  let result = 'Congratulation, your '+  pokemon.name + ' grew to LV.' + nextLevel + '!' 
  ...
  for (let i = 0; i < pokemon['evolutionLine'].length; i++) {
    ...
    if (pokemon.evolutionLine[i].level < nextLevel) {
      result += 'And it evolved into ' + pokemon['evolutionLine'][i].name
    }
    else if (pokemon['evolutionLine'][i].level === nextLevel) {
      result += ' and ' + pokemon['evolutionLine'][i].name
    }
  }
  return result
}
